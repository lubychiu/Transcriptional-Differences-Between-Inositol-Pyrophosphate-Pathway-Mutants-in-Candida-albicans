...
- Directional Gene Ontology Enrichment (clusterProfiler & GO.db): Splits significantly altered protein-coding genes by regulatory direction—Up-regulated versus Down-regulated—and queries the universal GO.db ontology map using a custom hypergeometric enrichment model to generate dotplots using true biological pathway names on the chart axes.
